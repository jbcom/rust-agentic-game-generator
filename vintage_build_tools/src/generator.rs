...
impl GameDataGenerator {
    ...
    pub fn with_openai_key(api_key: String, openai_api_key: String, timeline_start: i32, timeline_end: i32) -> Self {
        Self {
            api_key,
    ...
    pub async fn generate(&self) -> Result<()> {
        ...
        println!("Building vintage game timeline ({}-{})...", self.timeline_start, self.timeline_end);
        ...
        println!("Built timeline with {} exemplar games", timeline_games.len());
        ...
        let graph_data = GraphBuilder::build_enriched_game_graph(&timeline_games, &enriched_metadata)?;
    ...
    fn is_already_generated(&self) -> bool {
        ...
        let has_images = std::fs::read_dir(covers_dir)
            .map(|entries| {
    ...
    fn validate_generation(&self) -> Result<()> {
        ...
        for module in &required_modules {
            if !Path::new(module).exists() {
                anyhow::bail!("FATAL: Failed to generate {}! The wizard will not function without this module.", module);
            }
        }
        ...
        let covers_dir = Path::new("assets/wizard/game_covers");
        if !covers_dir.exists() {
            anyhow::bail!("FATAL: Failed to create game covers directory! The guided mode requires game images.");
        }
        ...
        if image_count == 0 {
            anyhow::bail!("FATAL: No game cover images were downloaded! The guided mode requires at least one game image.");
        }
        
        println!("✓ Validated {} modules and {} game cover images", required_modules.len(), image_count);
        Ok(())
    }
    
    /// Convert enhanced games to JSON format for templates
    fn convert_to_json(&self, enhanced_games: &[(i32, String, Game)]) -> Result<Vec<serde_json::Value>> {
        ...
        for (year, genre, game) in enhanced_games {
            ...
            game_data.insert("platforms".to_string(), serde_json::json!(
                game.platforms.as_ref()
                    .map(|p| p.iter().map(|platform| &platform.name).collect::<Vec<_>>())
                    .unwrap_or_default()
            ));
            game_data.insert("developers".to_string(), serde_json::json!(
                game.developers.as_ref()
                    .and_then(|d| d.first())
                    .map(|dev| &dev.name)
            ));
            ...
            if let Some(image) = &game.image {
                game_data.insert("image_icon_url".to_string(), serde_json::json!(image.icon_url));
                game_data.insert("image_medium_url".to_string(), serde_json::json!(image.medium_url));
                game_data.insert("image_screen_url".to_string(), serde_json::json!(image.screen_url));
                game_data.insert("image_screen_large_url".to_string(), serde_json::json!(image.screen_large_url));
                game_data.insert("image_small_url".to_string(), serde_json::json!(image.small_url));
                game_data.insert("image_super_url".to_string(), serde_json::json!(image.super_url));
                game_data.insert("image_thumb_url".to_string(), serde_json::json!(image.thumb_url));
                game_data.insert("image_tiny_url".to_string(), serde_json::json!(image.tiny_url));
                game_data.insert("image_original_url".to_string(), serde_json::json!(image.original_url));
            }
            
            game_data.insert("site_url".to_string(), serde_json::json!(
                game.site_detail_url.as_ref().unwrap_or(&"".to_string())
            ));
    ...
    fn merge_enriched_metadata(&self, timeline_games: &mut Vec<serde_json::Value>, enriched: &[EnrichedGameMetadata]) -> Result<()> {
        // Create a map of enriched metadata by game ID
        let enriched_map: std::collections::HashMap<u32, &EnrichedGameMetadata> = enriched
            .iter()
            .map(|e| (e.id, e))
            .collect();
        ...
        for game in timeline_games.iter_mut() {
            if let Some(game_obj) = game.as_object_mut() {
                if let Some(id) = game_obj.get("id").and_then(|v| v.as_u64()) {
                    if let Some(enriched_data) = enriched_map.get(&(id as u32)) {
                        ...
                        if let Some(enriched_obj) = enriched_json.as_object() {
                            ...
                            let skip_fields = ["id", "name", "year", "original_genre", "platforms", "developer", "deck"];
