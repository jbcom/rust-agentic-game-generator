...
impl ImageDownloader {
    ...
    pub fn download_game_covers(&self, timeline_games: &[serde_json::Value]) -> Result<()> {
        ...
        let download_tasks: Vec<(u32, String, PathBuf)> = timeline_games
            ...
            .filter_map(|game| {
                ...
                let image_url = game.get("image_small_url")
                    .and_then(|v| v.as_str())
                    .or_else(|| game.get("image_thumb_url").and_then(|v| v.as_str()))?;
                
                let image_path = Path::new(&self.images_dir).join(format!("{}.jpg", game_id));
                ...
                for (game_id, url, image_path) in chunk {
                    match Self::download_single_image(&client, url, image_path) {
                        Ok(true) => {
                            ...
                            if current % 10 == 0 {
                                println!("  Progress: {}/{} images downloaded", current, total_to_download);
                            }
                        }
                        Ok(false) => {
        ...
        println!("  Downloaded {} new game covers ({} already existed, {} failed)", 
                 final_downloaded, already_existed, final_failed);
    ...
    pub fn download_platform_logos(&self, platforms: &[crate::types::PlatformInfo]) -> Result<()> {
        ...
        if downloaded > 0 {
            println!("  Downloaded {} platform logos", downloaded);
        }
