...
//! - Embeddings and blend calculations
...
impl AiCache {
    ...
    pub fn generate_key(&self, prefix: &str, content: &str, params: &HashMap<String, String>) -> String {
    ...
    /// Put item in cache
    pub async fn put(&self, key: String, data: CachedData, params: HashMap<String, serde_json::Value>) -> Result<()> {
        let now = chrono::Utc::now();
...
impl ImageCache {
    ...
    pub async fn put_image(&self, key: String, data: Vec<u8>, params: HashMap<String, serde_json::Value>) -> Result<()> {
        // Optimize image before caching
        let optimized = self.optimize_image(&data)?;
        
        self.base_cache.put(
            key,
            CachedData::Image(optimized),
            params,
        ).await
    }
    ...
    fn convert_format(&self, data: &[u8], format: ImageFormat) -> Result<Vec<u8>> {
        ...
        match format {
            ImageFormat::Png => img.write_to(&mut std::io::Cursor::new(&mut buffer), image::ImageFormat::Png)?,
