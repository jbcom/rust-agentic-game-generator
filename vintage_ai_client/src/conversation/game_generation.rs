...
impl GameGenerationExt for ConversationManager {
    async fn start_game_design_conversation(
        ...
        if let Some(env) = self.template_env.lock().await.as_ref() {
            if let Ok(template) = env.get_template("01_game_design_system") {
        ...
        let conversation_id = self.start_conversation(
            format!("Game Design: {}", chrono::Utc::now().format("%Y-%m-%d %H:%M")),
            context
        ).await?;
        
        // Send initial prompt
        let response = self.send_message(&conversation_id, initial_prompt.to_string()).await?;
    ...
    async fn continue_game_design_conversation(
        ...
        let response = self.send_message(conversation_id, user_input.to_string()).await?;
        ...
        if is_complete {
            ...
            if let Some(env) = self.template_env.lock().await.as_ref() {
                if let Ok(template) = env.get_template("03_extract_game_config") {
                    let extraction_prompt = template.render(context!())?;
                    
                    let config_json = self.send_message_with_config(
                        conversation_id,
                        extraction_prompt,
                        Some(MessageConfig {
        ...
        F: Fn(GenerationProgress) + Send + 'static,
    {
        ...
        let style_guide = generate_style_guide(self, &conversation_id, config, project_config.as_ref()).await?;
        ...
        let world_data = generate_world(self, &conversation_id, config, &style_guide, project_config.as_ref()).await?;
...
async fn generate_style_guide(
    ...
    if let Some(env) = manager.template_env.lock().await.as_ref() {
        if let Ok(template) = env.get_template("05_style_guide") {
            ...
            let response = manager.send_message_with_config(
                conversation_id,
                prompt,
                Some(MessageConfig {
                    model: "gpt-4-turbo".to_string(),
...
async fn generate_world(
    ...
    if let Some(env) = manager.template_env.lock().await.as_ref() {
        if let Ok(template) = env.get_template("06_world_generation") {
            let prompt = if let Some(project) = project_config {
                template.render(context!(
                    project => project,
                    config => config,
