...
use std::path::Path;
use anyhow::Result;
use bevy_egui::egui::{ColorImage, TextureHandle, Context};
...
impl WizardImages {
    ...
    pub fn load_all(&mut self, ctx: &Context) {
        ...
        if let Ok(texture) = load_texture_from_path(
            ctx,
            format!("{}under_construction_overlay_transparent.png", base_path),
            "under_construction"
        ) {
        ...
        if let Ok(texture) = load_texture_from_path(
            ctx,
            format!("{}logo_main.png", base_path),
            "logo_main"
        ) {
        ...
        if let Ok(texture) = load_texture_from_path(
            ctx,
            format!("{}guided_mode_icon_transparent.png", base_path),
            "guided_mode_icon"
        ) {
        ...
        if let Ok(texture) = load_texture_from_path(
            ctx,
            format!("{}freeform_mode_icon.png", base_path),
            "freeform_mode_icon"
        ) {
