...
pub fn render_overlay(
    ...
    egui::Area::new(area_id)
        ...
        .show(ui.ctx(), |ui| {
            ...
            let response = ui.allocate_rect(config.rect.translate(-config.rect.min.to_vec2()), sense);
            let mut clicked_id = None;
            
            match &config.content {
                OverlayContent::Image { texture_id, tint } => {
                    // Use egui's Image widget for better integration
                    let image = egui::Image::from_texture((*texture_id, config.rect.size()))
                        .tint(if config.opacity < 1.0 {
                ...
                OverlayContent::Text { text, font_id, color, background, outline } => {
                    ...
                    if let Some((width, outline_color)) = outline {
                        ...
                        let offsets = [
                            (-*width, -*width), (0.0, -*width), (*width, -*width),
                            (-*width, 0.0),                      (*width, 0.0),
                            (-*width, *width),  (0.0, *width),  (*width, *width),
                        ];
                ...
                OverlayContent::Hotspot { id, hover_color, hover_stroke_width, tooltip } => {
...
pub fn render_overlay_stack(
    ui: &mut egui::Ui,
    overlays: &mut [OverlayConfig],
) -> Option<String> {
    // Sort by z_order
...
pub fn calculate_overlay_rect(
    image_rect: egui::Rect,
    area: egui::Rect,
    scale: f32,
) -> egui::Rect {
    let scaled_area = egui::Rect {
        min: egui::pos2(
...
pub fn show_image_with_overlays(
    ...
    for hotspot in &config.hotspots {
        ...
        if is_disabled {
            ...
            if hotspot_response.hovered() {
                painter.rect_stroke(rect, 4.0, egui::Stroke::new(2.0, hover_color), egui::epaint::StrokeKind::Outside);
            }
    ...
    for text_area in &config.text_areas {
        if let Some((_, text, font_id)) = text_overlays.iter().find(|(id, _, _)| *id == text_area.id) {
