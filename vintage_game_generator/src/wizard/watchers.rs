...
fn is_prompt_file(path: &PathBuf) -> bool {
    path.extension()
        ...
        match event.event_type {
            FileEventType::Created => {
                ...
                if event.path.to_string_lossy().contains("generated") {
                    ...
                    if let (Some(content), Some(file_name)) = (
                        ...
                        app_state.add_log(
                            LogLevel::Info,
                            format!("Added {} to validation queue", file_name)
                        );
                    }
                }
            }
            FileEventType::Modified => {
                ...
                if event.path.to_string_lossy().contains("validated") {
                    app_state.add_log(
                        LogLevel::Warning,
                        format!("Validated prompt was modified: {:?}", event.path.file_name())
                    );
                }
            }
