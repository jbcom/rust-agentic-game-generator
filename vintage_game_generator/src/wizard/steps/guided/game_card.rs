...
use super::types::{GuidedModeState, GameCardStyle};
        ...
        if state.selected_games.is_empty() {
            ...
            ui.horizontal(|ui| {
                ...
                if state.selected_games.len() >= 2 {
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if state.blend_result.is_some() {
                            if ui.button("🔄 Re-blend").clicked() {
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    ...
                    let selected_games: Vec<(u32, &'static TimelineGame)> = 
                        state.selected_games.iter().map(|(id, game)| (*id, *game)).collect();
                    
                    for (id, game) in selected_games {
                        // Create a temporary UI state to avoid mutable borrow conflicts
                        let removed = ui.group(|ui| {
...
fn render_complexity_bar(ui: &mut egui::Ui, complexity: f32) {
    ...
    // Complexity label
    ui.label(
        egui::RichText::new(format!("{:.0}%", complexity * 100.0))
            .small()
    );
}
