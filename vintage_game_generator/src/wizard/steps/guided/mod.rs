...
pub use types::{GuidedModeState, BlendResult, GuidedModeExport, Synergy, Conflict, SourceGame};
pub use timeline::render_timeline;
pub use game_card::render_game_card;
pub use blend::{
    render_blend_ui,
    create_blend,
    render_blend_visualization,
    render_export_ui,
    export_blend_to_config,
};

use bevy::prelude::*;
use bevy_egui::{egui, EguiContexts};
use crate::wizard::{
    state::{AppState, WizardStep},
};
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            match guided_state.current_step {
                0 => {
                    ...
                    if !guided_state.selected_games.is_empty() {
                        ui.separator();
                        ui.horizontal(|ui| {
                            ui.label(format!("Selected {} games", guided_state.selected_games.len()));
                ...
                1 => {
                    ...
                    if guided_state.blend_result.is_some() {
                        ...
                        if ui.button("✅ Export Configuration").clicked() {
                            if let Some(export) = export_blend_to_config(&guided_state) {
            ...
            if !guided_state.selected_games.is_empty() {
                ...
                egui::CollapsingHeader::new("Selected Games")
                    .default_open(true)
                    .show(ui, |ui| {
                        let game_list: Vec<(u32, String, i32)> = guided_state.selected_games
                            .iter()
