...
use vintage_blending_core::{
    graph::{GameGraph, BlendPath},
    similarity::SimilarityEngine,
    CompatibilityEdge
};
use crate::wizard::steps::guided::types::{GuidedModeState, BlendResult};

use super::metadata::{build_game_metadata, determine_art_styles};
use super::analysis::{
    analyze_synergies, analyze_conflicts,
...
fn generate_blend_result(
    ...
    let avg_complexity = games.iter()
        .map(|g| metadata[&g.id.to_string()].feature_vector.complexity)
        .sum::<f32>() / games.len() as f32;
    
    let avg_balance = games.iter()
        .map(|g| metadata[&g.id.to_string()].feature_vector.action_strategy_balance)
        .sum::<f32>() / games.len() as f32;
    
    // Extract synergies and conflicts from the blend path
    let synergies = blend_path.synergies.iter().map(|s| {
    ...
    let conflicts = blend_path.conflicts.iter().map(|c| {
...
fn generate_blend_description(
    ...
    let dominant_genre = genres.iter()
