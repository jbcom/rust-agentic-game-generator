...
use vintage_blending_core::types::{GameMetadata, Synergy as CoreSynergy, Conflict as CoreConflict};
use crate::vintage_games::TimelineGame;
use crate::wizard::steps::guided::types::{Synergy, Conflict};
...
pub fn analyze_synergies(
    ...
    // Platform synergies
    let shared_platforms: Vec<_> = game1.platforms.iter()
    ...
    if shared_mechanics > 0 {
        synergies.push(CoreSynergy {
            ...
            description: format!("{} shared game mechanics", shared_mechanics),
            strength: 0.6,
        });
    }
...
pub fn analyze_conflicts(
    ...
    let balance_diff = (meta1.feature_vector.action_strategy_balance - 
                       meta2.feature_vector.action_strategy_balance).abs();
    if balance_diff > 1.0 {
        conflicts.push(CoreConflict {
        ...
        if (game1.genre == *genre1 && game2.genre == *genre2) ||
           (game1.genre == *genre2 && game2.genre == *genre1) {
            conflicts.push(CoreConflict {
                ...
                description: format!("{} and {} have very different player expectations", genre1, genre2),
                severity: 0.6,
