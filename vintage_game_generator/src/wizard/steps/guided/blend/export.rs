...
pub fn export_blend_to_config(state: &crate::wizard::steps::guided::GuidedModeState) -> Option<GuidedModeExport> {
    let blend = state.blend_result.as_ref()?;
    
    // Convert source games from the blend path
    let source_games = blend.blend_path.games.iter()
        .filter_map(|game_id| {
            // game_id is a String, need to parse it to u32
            game_id.parse::<u32>().ok().and_then(|id| {
                state.selected_games.get(&id).map(|game| {
                    SourceGame {
...
pub fn export_to_toml(state: &crate::wizard::steps::guided::GuidedModeState) -> Option<String> {
    ...
    toml.push_str(&format!("action_strategy_balance = {:.2}\n\n", export.action_strategy_balance));
    ...
    for game in &export.source_games {
        ...
        if let Some(dev) = &game.developer {
            toml.push_str(&format!("developer = \"{}\"\n", dev));
        }
        toml.push_str("\n");
    }
    ...
    for (genre, weight) in sorted_genres {
        toml.push_str(&format!("{} = {:.2}\n", genre.to_lowercase().replace(' ', "_"), weight));
    }
    toml.push_str("\n");
...
pub fn export_to_json(state: &crate::wizard::steps::guided::GuidedModeState) -> Option<serde_json::Value> {
    let export = export_blend_to_config(state)?;
    let blend = state.blend_result.as_ref()?;
...
pub fn generate_ai_prompt(state: &crate::wizard::steps::guided::GuidedModeState) -> anyhow::Result<String> {
    let blend = state.blend_result.as_ref()
        .ok_or_else(|| anyhow::anyhow!("No blend result available"))?;
    let export = export_blend_to_config(state)
        .ok_or_else(|| anyhow::anyhow!("Failed to export config"))?;
    
    let mut env = Environment::new();
    
    // Load the blend game design template
    let template_content = std::fs::read_to_string("metaprompts/blend_game_design.jinja")
        .or_else(|_| std::fs::read_to_string("crates/vintage_game_generator/metaprompts/blend_game_design.jinja"))
    ...
    let serializable_blend = SerializableBlend {
        ...
        synergies: blend.synergies.iter().map(|s| SerializableSynergy {
            game1: s.game1.clone(),
            game2: s.game2.clone(),
            description: s.description.clone(),
            strength: s.strength,
        }).collect(),
        conflicts: blend.conflicts.iter().map(|c| SerializableConflict {
            game1: c.game1.clone(),
            game2: c.game2.clone(),
...
pub fn render_export_ui(ui: &mut egui::Ui, state: &crate::wizard::steps::guided::GuidedModeState) {
    ...
    ui.horizontal(|ui| {
        ...
        if ui.button("📋 Copy JSON").clicked() {
            if let Some(json) = export_to_json(state) {
                // TODO: Copy to clipboard
