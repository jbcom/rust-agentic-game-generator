...
use crate::wizard::{config::ProjectConfig, AppDirectories, SwitchModeEvent, AppMode};
use uuid::Uuid;
...
impl Default for ListModeState {
    fn default() -> Self {
        Self {
            projects: Vec::new(),
            selected_index: 0,
...
pub fn draw_list_ui(
    ...
    if let Ok(ctx) = contexts.ctx_mut() {
        ...
        ui.separator();
        ui.label(format!("Found {} projects in: {}", state.projects.len(), directories.base_dir.display()));
        });
    }
    ...
    if let Ok(ctx) = contexts.ctx_mut() {
        ...
        ui.horizontal(|ui| {
            ...
            if ui.button("Next ▶").clicked() && state.selected_index < state.projects.len().saturating_sub(1) {
                state.selected_index += 1;
            }
        });
        });
        ...
        egui::ScrollArea::horizontal().show(ui, |ui| {
            ui.horizontal(|ui| {
                ...
                for (i, (_, config, _)) in state.projects.iter().enumerate() {
                    ...
                    if ui.selectable_label(is_selected, &config.basic_info.name).clicked() {
                        new_index = Some(i);
                    }
                }
    ...
    if let Ok(ctx) = contexts.ctx_mut() {
        ...
        ui.horizontal(|ui| {
            unsafe {
                if ui.selectable_label(SELECTED_TAB == "details", "Project Details").clicked() {
                    SELECTED_TAB = "details";
                }
                if ui.selectable_label(SELECTED_TAB == "files", "Files").clicked() {
                    SELECTED_TAB = "files";
                }
                if ui.selectable_label(SELECTED_TAB == "preview", "Preview").clicked() {
...
fn draw_project_details(ui: &mut egui::Ui, state: &ListModeState) {
    if let Some((uuid, config, path)) = state.projects.get(state.selected_index) {
        ...
        ui.collapsing("Gameplay", |ui| {
            ui.label(format!("Core Mechanics: {}", config.gameplay.core_mechanics.join(", ")));
            ui.label(format!("Progression Type: {}", config.gameplay.progression_type));
            ui.label(format!("Gameplay Loop: {}", config.gameplay.gameplay_loop));
            ui.label(format!("Player Motivation: {}", config.gameplay.player_motivation));
        });
        
        ui.collapsing("Visual Style", |ui| {
            ui.label(format!("Reference Games: {}", config.visual_style.reference_games.join(", ")));
            ui.label(format!("Color Mood: {}", config.visual_style.color_mood));
            ui.label(format!("Sprite Size: {} px", config.visual_style.sprite_size));
        ...
        ui.collapsing("Technical", |ui| {
            ...
            ui.label(format!("Target Platforms: {}", config.technical.target_platforms.join(", ")));
        });
    } else {
...
fn draw_project_preview(ui: &mut egui::Ui, state: &ListModeState) {
    if let Some((_, config, _)) = state.projects.get(state.selected_index) {
        ...
        ui.group(|ui| {
            ...
            ui.label(format!("• Inspired by {}", config.visual_style.reference_games.join(", ")));
...
fn load_projects(base_dir: &PathBuf) -> Result<Vec<(Uuid, ProjectConfig, PathBuf)>, Box<dyn std::error::Error>> {
