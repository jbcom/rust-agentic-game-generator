...
impl GenerationPipeline {
    ...
    pub fn initialize_generator(&self, _api_key: String, directories: &AppDirectories) -> Result<()> {
...
pub fn process_generation_queue(
    ...
    if let Some(prompt) = prompt_to_validate {
        ...
        if validation_errors.is_empty() {
            ...
            app_state.add_log(
                LogLevel::Warning,
                format!("Validation errors for {}: {:?}", prompt.name, validation_errors)
            );
        }
    ...
    let all_validated = app_state.prompt_validation_queue
        .iter()
        .all(|p| p.validated);
    ...
    if all_validated && !app_state.prompt_validation_queue.is_empty() {
        ...
        app_state.add_log(
            LogLevel::Success,
            format!("Phase {:?} complete, advancing...", current_phase)
        );
