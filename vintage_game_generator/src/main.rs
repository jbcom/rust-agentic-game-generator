...
use uuid::Uuid;
use vintage_game_generator::wizard::{WizardPlugin, AppDirectories, AppMode};
...
fn create_ai_config(args: &Args) -> AiConfig {
    AiConfig {
        ...
        cache_ttl: 3600 * 24 * 7, // 1 week default
        timeout_secs: args.ai_timeout,
        optimize_costs: true, // Always optimize costs
        max_concurrent: 5, // Default concurrent requests
    }.validate() // Validate and clamp values
}
...
fn main() {
    ...
    let (project_dir, config_file) = match mode {
        AppMode::Generate => {
            ...
            let conf_file = args.config_file.unwrap_or_else(|| {
    ...
    println!("  Provider: {}", ai_config.ai_provider);
    println!("  Text Model: {}", ai_config.text_model);
    println!("  Image Model: {} ({})", ai_config.image_model, ai_config.image_quality);
    println!("  Temperature: {}", ai_config.temperature);
    println!("  Cache: {}", if ai_config.cache_enabled { "enabled" } else { "disabled" });
    println!();
    ...
    match mode {
        AppMode::List => {
            app.insert_resource(vintage_game_generator::wizard::list_mode::ListModeState::default());
        }
        AppMode::Generate => {
