...
fn game_to_metadata(game: &TimelineGame) -> GameMetadata {
    ...
    match game.genre {
        ...
        "RPG" | "Role-Playing" => {
            if let Some(idx) = STANDARD_MECHANICS.iter().position(|&m| m == "Character Progression") {
                mechanic_flags[idx] = true;
            }
            if let Some(idx) = STANDARD_MECHANICS.iter().position(|&m| m == "Exploration") {
                mechanic_flags[idx] = true;
            }
            mechanic_tags.extend_from_slice(&["Stats".to_string(), "Inventory".to_string(), "Quests".to_string()]);
        }
        "Strategy" => {
            if let Some(idx) = STANDARD_MECHANICS.iter().position(|&m| m == "Turn-Based") {
                mechanic_flags[idx] = true;
            }
            if let Some(idx) = STANDARD_MECHANICS.iter().position(|&m| m == "Resource Management") {
                mechanic_flags[idx] = true;
            }
        }
        "Platform" => {
            if let Some(idx) = STANDARD_MECHANICS.iter().position(|&m| m == "Platform Jumping") {
                mechanic_flags[idx] = true;
            }
        ...
        "Puzzle" => {
            if let Some(idx) = STANDARD_MECHANICS.iter().position(|&m| m == "Puzzle Solving") {
                mechanic_flags[idx] = true;
            }
...
pub fn find_similar_games(game_id: u32, count: usize) -> Vec<(&'static TimelineGame, f32)> {
    ...
    let mut similarities: Vec<(&'static TimelineGame, f32)> = TIMELINE_GAMES.iter()
...
pub fn build_era_subgraph(eras: &[Era]) -> Graph<GameNode, f32> {
    ...
    // Filter games by era
    let era_games: Vec<_> = TIMELINE_GAMES.iter()
        .filter(|game| {
