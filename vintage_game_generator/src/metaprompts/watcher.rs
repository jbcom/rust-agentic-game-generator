...
impl PromptWatcher {
    ...
    fn process_event(&mut self, event: Event, watcher_type: WatcherType) -> Option<FileChangeEvent> {
        match event.kind {
...
impl GenerationTask {
    ...
    fn is_similar(&self, other: &GenerationTask) -> bool {
        match (&self.task_type, &other.task_type) {
            (GenerationTaskType::ValidatePrompt(a), GenerationTaskType::ValidatePrompt(b)) => a == b,
            (GenerationTaskType::GenerateAssets(a), GenerationTaskType::GenerateAssets(b)) => a == b,
