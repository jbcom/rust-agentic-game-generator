...
use vintage_game_generator::{
    wizard::{AppState, AppMode, AppDirectories},
    vintage_games,
};
...
fn test_app_directories() {
    ...
    directories.ensure_directories_exist().expect("Failed to create directories");
...
fn test_asset_paths() {
    ...
    let exists = asset_paths.iter().any(|path| {
...
#[test]
#[cfg_attr(target_os = "macos", ignore = "macOS requires EventLoop on main thread")]
...
fn test_wizard_plugin_resources() {
    use vintage_game_generator::wizard::{WizardPlugin, GenerationPipeline};
    ...
    assert!(app.world().get_resource::<AppState>().is_some(), "AppState should be added by WizardPlugin");
    assert!(app.world().get_resource::<GenerationPipeline>().is_some(), "GenerationPipeline should be added by WizardPlugin");
}
...
fn test_draw_generate_ui_system_exists() {
    ...
    assert!(true, "WizardPlugin should register draw_generate_ui system");
}
...
fn test_egui_context_error_handling() {
    ...
    assert!(error_msg.contains("NoEntities"), "Error should indicate missing entities");
    assert!(error_msg.contains("EguiContext"), "Error should mention EguiContext");
}
...
fn test_asset_configuration_files() {
    ...
    for ron_file in ron_files {
        if std::path::Path::new(ron_file).exists() {
            ...
            assert!(!content.is_empty(), "RON file should not be empty: {}", ron_file);
        }
    }
}
...
fn test_png_assets_exist() {
    ...
    if found_count == 0 {
        ...
        println!("Found {} out of {} PNG assets", found_count, png_files.len());
    }
}
...
fn test_metaprompt_templates() {
    ...
    for template in &templates {
        if std::path::Path::new(template).exists() {
            ...
            assert!(!content.is_empty(), "Template should not be empty: {}", template);
    ...
    if found_count == 0 {
        ...
        println!("Found {} out of {} metaprompt templates", found_count, templates.len());
    }
}
...
fn test_vintage_timeline_json() {
    ...
    if std::path::Path::new(timeline_path).exists() {
        let content = std::fs::read_to_string(timeline_path)
            .expect("Failed to read timeline JSON");
        
        // Verify it's valid JSON
        let parsed: serde_json::Value = serde_json::from_str(&content)
            .expect("Timeline JSON should be valid");
...
fn test_wizard_state_transitions() {
    ...
    // Test initial state
    assert_eq!(app_state.wizard_step, vintage_game_generator::wizard::state::WizardStep::Welcome);
    
    // Test welcome action
    app_state.set_wizard_mode(WelcomeAction::GuidedMode);
    assert_eq!(app_state.wizard_step, vintage_game_generator::wizard::state::WizardStep::SelectLanguage);
