...
impl MetadataBuilder {
    ...
    pub fn build_from_json(&self, game_data: &Value) -> Result<GameMetadata> {
        let game_id = game_data["guid"].as_str()
            .ok_or_else(|| anyhow::anyhow!("Missing game guid"))?
            .to_string();
            
        let name = game_data["name"].as_str()
            .ok_or_else(|| anyhow::anyhow!("Missing game name"))?
            .to_string();
            
        let year = game_data["year"].as_u64()
        ...
        for game_id in &game_ids {
            ...
            if let Some(game_meta) = metadata.get(game_id) {
                for other_id in &game_ids {
                    if game_id != other_id {
                        if let Some(other_meta) = metadata.get(other_id) {
                            let compatibility = game_meta.feature_vector.similarity(&other_meta.feature_vector);
