...
use crate::types::{GameMetadata, CompatibilityEdge, Conflict, Synergy};
...
impl GameGraph {
    ...
    pub fn find_blend_path(&self, game_ids: &[String]) -> Result<BlendPath> {
        ...
        for game_id in game_ids {
            if !self.node_lookup.contains_key(game_id) {
                anyhow::bail!("Game {} not found in graph", game_id);
            }
    ...
    pub fn analyze_edge(&self, game1_id: &str, game2_id: &str) -> Result<CompatibilityEdge> {
        let meta1 = self.metadata.get(game1_id)
            .ok_or_else(|| anyhow::anyhow!("Game {} not found", game1_id))?;
        let meta2 = self.metadata.get(game2_id)
            .ok_or_else(|| anyhow::anyhow!("Game {} not found", game2_id))?;
        ...
        let balance_diff = (meta1.feature_vector.action_strategy_balance - meta2.feature_vector.action_strategy_balance).abs();
        if balance_diff > 1.0 {
            conflicts.push(Conflict {
                type_name: "Gameplay Style Conflict".to_string(),
                description: "One game is action-focused while the other is strategy-focused".to_string(),
                severity: balance_diff / 2.0,
                resolution_hint: "Blend by creating strategic action sequences or real-time strategy elements".to_string(),
            });
        }
